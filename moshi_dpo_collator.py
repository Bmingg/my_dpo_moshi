...
class SpokenSwagMoshiCollator:
    ...
    def __call__(self, features: list[dict]) -> dict:
        ...
        if self.mimi_encoder is not None:
            with torch.no_grad():
                # MimiModel.encode returns an output with `audio_codes` of
                # shape [B, num_codebooks, T_frames]. Uses the encoder's device.
                c_moshi_codes = self.mimi_encoder.encode(
                    c_moshi_wav.unsqueeze(1).to(next(self.mimi_encoder.parameters()).device)
                ).audio_codes
                c_user_codes = self.mimi_encoder.encode(
                    c_user_wav.unsqueeze(1).to(next(self.mimi_encoder.parameters()).device)
                ).audio_codes
                r_moshi_codes = self.mimi_encoder.encode(
                    r_moshi_wav.unsqueeze(1).to(next(self.mimi_encoder.parameters()).device)
                ).audio_codes
                r_user_codes = self.mimi_encoder.encode(
                    r_user_wav.unsqueeze(1).to(next(self.mimi_encoder.parameters()).device)
                ).audio_codes
