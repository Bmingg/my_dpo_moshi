...
class SLAMMoshiDPOTrainer(DPOTrainer):
    ...
    def _side_logps(
        ...
        # to skip the internal Mimi encoding. No `labels` are passed — we want
        # raw logits so we can gather per-example log-probs ourselves.
        outputs = model(
            input_ids=text_ids,
    ...
    def verify_manual_loss_matches_hf(
        ...
        # Re-run forward without labels to get plain logits.
        # (Some HF models compute loss even if labels are None; we just want the logits either way.)
        outputs = model(
            input_ids=text_ids,
            attention_mask=attention_mask,
            user_audio_codes=user_codes,
