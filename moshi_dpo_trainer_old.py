...
class SLAMMoshiDPOTrainer(DPOTrainer):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        base = self.model
        mimi = base.audio_encoder
        mimi.eval()
        for param in mimi.parameters():
            param.requires_grad = False
        logger.info("Mimi encoder frozen (%d params)", 
                sum(p.numel() for p in mimi.parameters()))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """Override TRL's compute_loss to route through Moshi's dual-stream audio.

        TRL's default _compute_loss calls model(**kwargs) directly, which fails
        because our batch keys (chosen_moshi_audio_codes, etc.) don't match
        MoshiForConditionalGeneration's forward signature. We intercept here
        and compute the DPO loss ourselves.
        """
        audio_bos = self._resolve_audio_bos(
        ...
        # store_metrics is TRL's batched logging method
    ...
    def _prepare_dataset(self, dataset, *args, **kwargs):
        """Bypass TRL's text-based dataset preparation (EOS appending, etc.)."""
        return dataset
